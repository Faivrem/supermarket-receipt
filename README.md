# Supermarket-receipt kata

## Bagdes

![](https://travis-ci.org/Faivrem/supermarket-receipt.svg?branch=master)
[![codecov](https://codecov.io/gh/Faivrem/supermarket-receipt/branch/master/graph/badge.svg)](https://codecov.io/gh/Faivrem/supermarket-receipt)

## Membres

- CARLU Ludovic - LudoCarlu
- FAIVRE Maxime - Faivrem
- LE BORGNE Kevin - lebnet
- TURRATA Augustin - Gustutu

## Avancement

- [x] TP1
- [x] TP2 
- [x] TP3 
- [x] TP4

###Fonctionnalités TP4
2 / 6 : Ajout d'un produit et consultation de la liste des produits

## Informations

### Codecov

Codecov n'est pas à 100% en raison de l'implémentation de l'application web String Boot

### Tests

Les tests l'application Spring Boot ne sont pas faits
